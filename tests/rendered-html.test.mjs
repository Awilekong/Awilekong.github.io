...
test("renders the v1 layout with the selected enhancements", async () => {
  ...
  assert.doesNotMatch(profilePhoto, /profile-wink\.jpg/);
  assert.doesNotMatch(profilePhoto, /revealTouchWink/);
  assert.doesNotMatch(profilePhoto, /onPointerDown/);
  assert.doesNotMatch(styles, /profile-photo-wink/);
  assert.doesNotMatch(styles, /is-touch-winking/);
